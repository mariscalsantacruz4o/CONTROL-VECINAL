...
function cleanNoticeImage(value: unknown) {
  const imageUrl = String(value ?? "").trim();
  if (!imageUrl || imageUrl.startsWith("blob:")) return "";
  if (imageUrl.length > 800_000) throw new Error("La fotografía del aviso es demasiado grande");
  const isEmbeddedImage = /^data:image\/(?:jpeg|png|webp);base64,[a-z0-9+/=]+$/i.test(imageUrl);
  const isSecureUrl = /^https:\/\/[^\s]+$/i.test(imageUrl);
  if (!isEmbeddedImage && !isSecureUrl) throw new Error("La fotografía del aviso no tiene un formato válido");
  return imageUrl;
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const imageUrl = cleanNoticeImage(body.imageUrl);
    ...
    await d1.batch([
      ...
      `).bind(title, cleanText(body.body, 800), body.active === false ? 0 : 1, imageUrl, cleanText(body.eventType, 80), eventDate, cleanText(body.eventTime, 5), cleanText(body.eventPlace, 160), cleanText(body.whatsapp, 30), now),
